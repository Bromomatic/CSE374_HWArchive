...
// the node containing that word if possible. Modify the output string based on our findings.
struct Node* search_trie(char* input, struct Node* tracker, char* output) {
	...
	int m = 0;
	while ((input[m] != '\0') && (tracker != NULL)) {
		int direction;		
		direction = input[m] - '0';
		
		// Search for t9onyms if given a #, otherwise search based on direction.
		if (input[m] == '#') {
			if (tracker->next[0] != NULL) {
				tracker = tracker->next[0];
			} else {
				strcpy(output, "There are no more T9onyms");
				return tracker;
			}
		} else {
			if (tracker->next[direction] == NULL) { 
				strcpy(output, "Not found in current dictionary.");
				return tracker;
			}
	...
	// Prevent from reading a NULL node if searching managed to get us off the trie
	// or to a node that is only part of the trie and doesn't contain a word.
	if (tracker == NULL || tracker->word == NULL) {
		strcpy(output, "Not found in current dictionary.");
	} else {
		strcpy(output, tracker->word);
	}
